<template>
  <el-dialog :visible.sync="show" title="新增礼品信息" width="800px" :close-on-click-modal="false" @close="onDialogClose">
    <div v-loading="loading" class="edit-form-wrapper">
      <el-form size="small" :model="model" :rules="rules" label-position="left" label-width="120px" ref="createForm">
        ...
        <el-form-item label="礼品图片" prop="img">
          <!-- 图片文件上传 -->
          <el-upload :action="uploadAction" :on-success="onUploadSuccess" :on-error="onUploadError" :file-list="imgFileList" list-type="picture" :before-remove="onFileRemove" ref="img-upload">
...
<script>
import { createGiftInfo } from '../../../api/gift/gift-info';
import { emptyGiftInfo } from '../../../utils/empty-model';

export default {
  name: 'gift-info-create-dialog',
  ...
  methods: {
    ...
    doCreate() {
      this.$refs.createForm.validate(valid => {
        if (valid) {
          createGiftInfo(this.model)
            .then(data => {
              this.$message.success('创建礼品信息成功');
    ...
    onSvgaUploadSuccess(response, file, fileList) {
      ...
      this.$refs.createForm.validateField('svgaUrl');
      this.$message.success('上传成功!');
    },
