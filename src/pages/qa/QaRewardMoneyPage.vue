<template>
  <div>
    ...
    <div class="btn-wrapper">
      ...
      <!-- 批量删除按钮 -->
      <!-- 如果没有选择任何一行, 则按钮无效 -->
      <el-button @click="deleteBatch" type="danger" size="small" v-if="hasPermission('qa:reward-money:delete')" class="btn-operation" :disabled="selectedIds.length === 0">
        <i class="fa fa-trash"></i>
        <span>批量删除</span>
      </el-button>
      <!-- 新增礼品信息按钮 -->
      <el-button @click="openCreateDialog" type="success" size="small" v-if="hasPermission('qa:reward-money:save')" class="btn-operation">
    ...
    <el-table :data="tableData" border style="width: 100%" v-loading="loading.table" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" @selection-change="onSelectionChange" @sort-change="onSortChange">
      ...
      <el-table-column prop="rewardId" label="ID" sortable="custom"></el-table-column>
      ...
      <el-table-column fixed="right" label="操作">
        <template slot-scope="scope">
          <el-button-group>
            <el-tooltip class="item" effect="dark" content="编辑" placement="top" v-if="hasPermission('qa:reward-money:update')">
              <el-button type="warning" size="mini" @click="openEditDialog(scope.row.rewardId)">
                <i class="fa fa-edit"></i>
              </el-button>
            </el-tooltip>
            <el-tooltip class="item" effect="dark" content="删除" placement="top" v-if="hasPermission('qa:reward-money:delete')">
    ...
    <!-- 弹窗 start-->
    <!-- 修改悬赏金额 start-->
    <el-dialog :visible.sync="dialog.edit.show" title="悬赏金额修改" width="400px">
      <div v-loading="dialog.edit.loading" class="edit-form-wrapper">
        <el-form size="small" :model="dialog.edit.model" :rules="dialog.edit.rules" label-position="left" label-width="80px" ref="editForm">
          <el-form-item label="悬赏金额" prop="price">
            <el-input v-model.number="dialog.edit.model.price" type="number" min="1"></el-input>
          </el-form-item>
        </el-form>
      </div>
      <span slot="footer">
    ...
    <!-- 修改悬赏金额 end-->
    <!-- 创建悬赏金额 start-->
    <el-dialog :visible.sync="dialog.create.show" title="悬赏金额创建" width="400px">
      <div v-loading="dialog.create.loading" class="create-form-wrapper">
        <el-form size="small" :model="dialog.create.model" :rules="dialog.create.rules" label-position="left" label-width="80px" ref="createForm">
          <el-form-item label="悬赏金额" prop="price">
            <el-input v-model.number="dialog.create.model.price" type="number" min="1"></el-input>
          </el-form-item>
        </el-form>
      </div>
      <span slot="footer">
...
<script>
import {
  getQaRewardMoneyList,
  getQaRewardMoneyDetail,
  updateQaRewardMoney,
  removeQaRewardMoney,
  createQaRewardMoney
} from '../../api/qa/qa-reward-money';

import { emptyQaRewardMoney } from '../../utils/empty-model';

export default {
  name: 'qa-reward-money-page',
  data() {
    return {
      selectedIds: [],
      dialog: {
        edit: {
          model: emptyQaRewardMoney(),
          rules: {
            price: [
              {
                required: true,
  ...
  methods: {
    ...
    deleteBatch() {
      this.$confirm('此操作将删除所选择的悬赏金额', '批量删除确认')
        .then(() => {
          this.loading.table = true;
          removeQaRewardMoney(this.selectedIds)
            .then(({ data }) => {
    ...
    doCreate() {
      ...
      this.$refs.createForm.validate(valid => {
        if (valid) {
          createQaRewardMoney(this.dialog.create.model)
            .then(data => {
              this.$message({
                type: 'success',
                message: '创建成功'
    ...
    doDeleteSingle(row) {
      this.$confirm('此操作将删除悬赏金额=' + row.price + '的记录', '删除确认')
        .then(() => {
          this.loading.table = true;
          removeQaRewardMoney(row.rewardId)
            .then(({ data }) => {
              this.$message.success('删除成功');
              // 刷新表格数据
              this.getTableData();
    ...
    doEdit() {
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          updateQaRewardMoney(this.dialog.edit.model)
            .then(data => {
              this.$message({
                type: 'success',
                message: '更新成功'
    ...
    openEditDialog(rewardId) {
      // 打开对话框
      this.dialog.edit.show = true;
      // 表单loading
      this.dialog.edit.formLoading = true;
      // 根据ID获取详情
      getQaRewardMoneyDetail(rewardId)
        .then(({ data }) => {
    ...
    getTableData() {
      // 显示表格loading
      this.loading.table = true;
      this.dialog.create.model = emptyQaRewardMoney();
      getQaRewardMoneyList({
