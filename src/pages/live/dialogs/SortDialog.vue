 <template>
  <el-dialog width="800px" title="直播录播文件排序" :close-on-click-modal="false" :visible.sync="show">
    <el-row class="tips">
      <p class="tip-text">
        ...
        <draggable class="list-group" element="ul" v-model="gifts" :options="dragOptions" @start="isDragging=true" @end="isDragging=false">
          <transition-group type="transition" :name="'flip-list'">
            <li class="list-group-item" v-for="item in gifts" :key="item.sort">
              ...
              <span class="type-name">{{item.id}}</span>
              <span class="type-name">直播记录ID:{{item.liveId}}</span>
