<template>
  <div>
    ...
    <div class="btn-wrapper">
      <el-button @click="getTableData" type="primary" size="small">
        <i class="fa fa-refresh"></i>
        <span>刷新表格数据</span>
      </el-button>
      <el-button @click="deleteBatch" type="danger" size="small" v-if="hasPermission('user:recommend:type:delete')" class="btn-operation" :disabled="selectedIds.length === 0">
      ...
      <el-table-column fixed="right" label="操作">
        <template slot-scope="scope">
          <el-button-group>
            ...
            <el-tooltip class="item" effect="dark" content="删除" placement="top" v-if="hasPermission('user:recommend:type:delete')">
...
<script>
import {
  getUserRecommendTypeList,
  removeUserRecommendType
} from '../../api/user/user-recommend-type';
import UserRecommendTypeCreateDialog from './dialogs/UserRecommendTypeCreateDialog';
import UserRecommendTypeEditDialog from './dialogs/UserRecommendTypeEditDialog';

export default {
  name: 'user-recommend-type-page',

  components: {
    'create-dialog': UserRecommendTypeCreateDialog,
    'edit-dialog': UserRecommendTypeEditDialog
  },
  ...
  methods: {
    ...
    deleteBatch() {
      this.$confirm('此操作将删除所选择的用户推荐类型', '批量删除确认')
        .then(() => {
          // 表格loading
          this.loading.table = true;
          removeUserRecommendType(this.selectedIds)
            .then(({ data }) => {
    ...
    deleteSingle(row) {
      this.$confirm(`是否要删除用户推荐类型: ${row.name} ?`, '删除确认')
        .then(() => {
          this.loading.table = true;
          removeUserRecommendType(row.typeId)
            .then(({ data }) => {
              this.$message.success('删除成功');
              // 刷新表格数据
              this.getTableData();
