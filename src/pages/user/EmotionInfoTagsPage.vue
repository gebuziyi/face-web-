<template>
  <div>
    ...
    <div class="btn-wrapper">
      ...
      <el-button @click="deleteBatch" type="danger" size="small" v-if="hasPermission('emotion:tags:delete')" class="btn-operation" :disabled="selectedIds.length === 0">
        <i class="fa fa-trash"></i>
        <span>批量删除</span>
      </el-button>
      <el-button @click="openCreateDialog" type="success" size="small" v-if="hasPermission('emotion:tags:save')" class="btn-operation">
    ...
    <el-table :data="tableData" border style="width: 100%" v-loading="loading.table" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" @selection-change="onSelectionChange" @sort-change="onSortChange">
      ...
      <el-table-column prop="tagsId" label="ID" sortable="custom"></el-table-column>
      ...
      <el-table-column fixed="right" label="操作">
        <template slot-scope="scope">
          <el-button-group>
            <el-tooltip class="item" effect="dark" content="编辑" placement="top" v-if="hasPermission('emotion:tags:update')">
              <el-button type="warning" size="mini" @click="openEditDialog(scope.row)">
                <i class="fa fa-edit"></i>
              </el-button>
            </el-tooltip>
            <el-tooltip class="item" effect="dark" content="删除" placement="top" v-if="hasPermission('emotion:tags:delete')">
    ...
    <!-- 弹窗 -->
    <!-- 修改情感状态标签信息 -->
    <el-dialog :visible.sync="dialog.edit.show" title="修改情感状态标签信息" width="600px" :close-on-click-modal="false">
    ...
    <!-- 创建情感状态标签信息 -->
    <el-dialog :visible.sync="dialog.create.show" title="创建情感状态标签信息" width="600px" :close-on-click-modal="false">
...
<script>
import {
  getEmotionInfoTagsList,
  getEmotionInfoTagsDetail,
  updateEmotionInfoTags,
  removeEmotionInfoTags,
  createEmotionInfoTags
} from '../../api/user/emotion-info-tags';

import { emptyEmotionInfoTags } from '../../utils/empty-model';
...
export default {
  ...
  methods: {
    ...
    onSelectionChange(rows) {
      if (rows) {
        this.selectedIds = rows.map(data => data.tagsId);
      }
    },
    deleteBatch() {
      this.$confirm(
        '此操作将删除所选择的情感状态标签信息列表, 用户与这些标签的关联记录也将一并删除, 确认继续?',
        '批量删除确认',
        {
          type: 'warning'
        }
        ...
        .then(() => {
          ...
          removeEmotionInfoTags(this.selectedIds)
            .then(({ data }) => {
    ...
    doCreate() {
      ...
      this.$refs.createForm.validate(valid => {
        if (valid) {
          createEmotionInfoTags(this.dialog.create.model)
            .then(data => {
    ...
    doDeleteSingle(row) {
      this.$confirm(
        ...
        .then(() => {
          this.loading.table = true;
          removeEmotionInfoTags(row.tagsId)
            .then(({ data }) => {
              this.$message.success('删除成功');
              // 刷新表格数据
              this.getTableData();
    ...
    doEdit() {
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          updateEmotionInfoTags(this.dialog.edit.model)
            .then(data => {
              this.$message.success('操作成功');
    ...
    openEditDialog(row) {
      ...
      getEmotionInfoTagsDetail(row.tagsId)
        .then(({ data }) => {
    ...
    getTableData() {
      // 显示表格loading
      this.loading.table = true;
      this.dialog.create.model = emptyEmotionInfoTags();
      getEmotionInfoTagsList({
