...
<script>
import { getPayTypeDiamondCountAuditData } from '../../../api/audit/payTypeDiamondCount-audit';

export default {
  name: 'payType-diamondCount-audit-page',
  ...
  methods: {
    refreshChartData() {
      ...
      getPayTypeDiamondCountAuditData({
        pager: this.pager
      })
        .then(({ data }) => {
          // 隐藏加载动画
          ...
          this.option = {
            ...
            legend: {
              orient: 'vertical',
              left: 'left',
              data: data.pieData.map(item => item.payName)
            },
            series: [
              {
                name: '钻石数量',
