...
<script>
import { getThanksgivingVideoByDailyLineLine } from '../../../../../api/audit/thanksgiving-video-audit';
...
export default {
  ...
  methods: {
    initLineChartData() {
      ...
      getThanksgivingVideoByDailyLineLine()
        .then(({ data }) => {
          // 隐藏加载动画
            ...
            series: [
              {
                name: '新增视频数量',
                type: 'line',
                data: data.newRegistList.map(data => data.videosNum),
                itemStyle: {
                  normal: {
                    label: {
                      show: true
