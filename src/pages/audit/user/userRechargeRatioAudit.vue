...
<script>
import { getuserRechargeRatioAudit } from '../../../api/audit/user-RechargeRatioAudit';

export default {
  name: 'user-RechargeRatio-audit-page',
    ...
    refreshChartData() {
      ...
      getuserRechargeRatioAudit({
        ...
        .then(({ data }) => {
          ...
          pieData.push({
            name: '充值会员',
            value: data.pieData.rechargeUserCount,
            // 默认选中充值会员饼图
            selected: true
          });
          pieData.push({
            name: '普通会员',
            value: data.pieData.normalUserCount
          });
          this.option = {
            title: {
              text: '用户充值比例统计',
