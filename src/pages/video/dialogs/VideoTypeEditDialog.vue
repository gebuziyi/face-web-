...
export default {
  ...
  methods: {
    getDetail(typeId) {
      getVideoTypeDetail(typeId)
        .then(({ data }) => {
          this.imgFileList = [].concat({ url: data.detail.img });
          this.bgImgFileList = [].concat({ url: data.detail.backgroundImg });
    ...
    onBgImgUploadSuccess(response, file, fileList) {
      ...
      if (response.code && response.code !== 0) {
        ...
        this.bgImgFileList = [].concat({ url: this.model.backgroundImg });
        return;
      }
      // 上传文件在服务器上的地址, 设置响应model的img属性值
      this.model.backgroundImg = response;
      this.bgImgFileList = [].concat({ url: response });
      this.$refs.editForm.validateField('backgroundImg');
      this.$message.success('上传成功!');
    },
