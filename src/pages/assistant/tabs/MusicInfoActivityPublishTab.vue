<template>
  <div>
    <div class="activity-form-wrapper">
      <h3>发布音乐信息活动</h3>
      <el-form class="activity-form" size="small" :model="model" :rules="rules" label-position="left" label-width="120px" ref="musicForm">
        <el-form-item label="活动介绍" prop="text">
          <el-input v-model.trim="model.text" type="textarea"></el-input>
        </el-form-item>
        <el-form-item prop="id" label="音乐信息">
          <el-select v-model="model.id" placeholder="键入搜索音乐名称" filterable :filter-method="remoteSearchMusicListByName" remote :loading="loading.select.mname">
            <template v-for="item in musicList">
              <el-option :label="item.mname" :value="item.musicId" :key="item.musicId"></el-option>
            </template>
          </el-select>
        </el-form-item>
        <el-form-item label="">
    ...
    <div class="tab-wrapper">
      ...
      <el-table :data="tableData" border style="width: 100%" v-loading="loading.table" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" @sort-change="onSortChange">
        ...
        <el-table-column prop="msgData" label="音乐ID" width="80">
          <template slot-scope="scope">
            <span>{{ scope.row | parseId }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="msgData" label="音乐名称" width="150">
          <template slot-scope="scope">
            <span>{{ scope.row | parseName }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="msgData" label="音乐封面" width="80">
          <template slot-scope="scope">
      ...
      <el-pagination @size-change="onSizeChange" @current-change="onCurrentPageChange" :current-page="pager.page" :page-sizes="[10, 20, 30]" :page-size="pager.limit" layout="total, sizes, prev, pager, next, jumper" :total="pager.total">
               ...
               <!-- 弹窗 -->
    <!-- 修改系统参数 -->
    <el-dialog :visible.sync="dialog.edit.show" title="修改活动介绍" width="1200px">
...
<script>
import { sendMusicMsg } from '../../../api/assistant/official-activity';
import { debounce } from 'lodash';
import { searchMusicListByName } from '../../../api/fuzzy-search';
import {
  getAssistantChatMsgByMusicPage,
  deleteMsg,
  getDetailList,
  updateSysConfig
...
export default {
  ...
  methods: {
    deleteActivityMsg(row) {
      this.$confirm(
        `此操作将删除音乐活动消息, 删除后所有用户的小助手页面都将看不到此消息, 是否继续?`,
        '删除音乐活动消息确认',
        {
          type: 'warning'
        }
