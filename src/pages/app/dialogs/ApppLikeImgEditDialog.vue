<template>
  <el-dialog :visible.sync="show" title="修改app点赞图标" width="800px" @close="onDialogClose($refs.createForm)" :close-on-click-modal="false">
...
export default {
  ...
  methods: {
    ...
    doGetAppLikeImgDetail(id) {
      getAppLikeImgDetail(id)
        .then(({ data }) => {
          this.model = data.detail;
          this.imgFileList = [].concat({ url: this.model.imgUrl });
          this.show = true;
        })
        .catch(error => {
          // do something
